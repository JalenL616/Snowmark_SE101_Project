document.addEventListener('DOMContentLoaded', function() {
    ...
    // --- UPDATED: A much more robust validation function ---
    function validateRow(row) {
        const errorMessages = [];
        // Find all inputs that have validation constraints (required, min, max, etc.)
        ...
        inputsToValidate.forEach(input => {
            ...
            // This checks required, min, max, and type constraints all at once.
            if (!input.checkValidity()) {
                let message = '';
                const fieldName = input.name.replace('_', ' ');
                const formattedName = fieldName.charAt(0).toUpperCase() + fieldName.slice(1);

                // Create a specific error message based on the type of error
                if (input.validity.valueMissing) {
                    ...
                    message = `Invalid value for ${formattedName}.`;
                }
